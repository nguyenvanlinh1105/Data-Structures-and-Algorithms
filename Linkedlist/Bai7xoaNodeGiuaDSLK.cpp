...
void pushFront(node **head,int x){
	// **head se tro den con tro dau tien . *head chinh la dia chi cua con tro dau tien.
...
void popCenter(node** head, int k){
	int n= count(*head);
	...
	if(k==1){
		...
		node *kNode = temp->next;// node thu k
		// no k-1 k?t noi voi k+1;
